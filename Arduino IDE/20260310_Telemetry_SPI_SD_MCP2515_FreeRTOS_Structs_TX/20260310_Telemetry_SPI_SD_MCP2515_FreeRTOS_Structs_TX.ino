...
PacoteTelemetria pacote_tx; // Instância global do pacote
...
// ==========================================
// VARIÁVEIS CAN (VOLATILE - Agora como INTEIROS)
// ==========================================
...
// ==========================================
// FUNÇÃO PARA CALCULAR O CHECKSUM (XOR simples)
// ==========================================
...
void setup() {
  ...
  Serial2.begin(115200, SERIAL_8N1, RXD2, TXD2); // ATENÇÃO: Baudrate LoRa ajustado
  ...
  // Inicializa o cabeçalho constante do pacote
...
void loop() {
  ...
  if (millis() - tempo_ultima_leitura >= intervalo_telemetria) {
    ...
    // ========================================================
    // 1. LEITURA DOS DADOS DO NANO VIA VSPI (AGORA EM BINÁRIO)
    // ========================================================
    const int tamanho_nano = 16; // Exatos 16 bytes que o Nano envia
    uint8_t buffer_nano[tamanho_nano];
    
    SPI.beginTransaction(SPISettings(1000000, MSBFIRST, SPI_MODE0));
    digitalWrite(pino_cs_nano, LOW);
    delayMicroseconds(10); 

    for (int i = 0; i < tamanho_nano; i++) {
      buffer_nano[i] = SPI.transfer(0x00); 
      delayMicroseconds(10); 
    }
    ...
    // Copia os 16 bytes recebidos do Nano DIRETAMENTE para o espaço de memória da struct!
    // Como pacote_tx.a0 é o início do bloco do Nano, o memcpy joga as 8 variáveis de uma vez só.
    memcpy(&pacote_tx.a0, buffer_nano, tamanho_nano);

    // Popula com as variáveis CAN captadas no Núcleo 0
    ...
    // ========================================================
    // 3. ENVIO VIA LORA (UART2) - 73 Bytes
    // ========================================================
    Serial2.write((uint8_t*)&pacote_tx, sizeof(PacoteTelemetria));

    // ========================================================
    // 4. GRAVAÇÃO NO CARTÃO SD (VSPI) - AGORA EM BINÁRIO!
    // ========================================================
    // Mudamos a extensão para .bin (pode ser .log ou .dat também)
    File arquivo = SD.open("/telemetry.log", FILE_APPEND);
    if (arquivo) {
      // Usamos .write() passando o ponteiro da memória e o tamanho exato, ao invés de .println()
