...
const int TAMANHO_PACOTE = sizeof(PacoteTelemetria); // 73 bytes originais
const int TAMANHO_LORA = TAMANHO_PACOTE + 1;         // 74 bytes (73 dados + 1 RSSI injetado)

// Buffer agora tem 74 bytes
uint8_t buffer_rx[TAMANHO_LORA]; 
int indice_buffer = 0;
...
void loop() {
  ...
  while (Serial2.available() > 0) {
    ...
    // Passo 2: Cabeçalho encontrado, ler o resto do pacote
      ...
      // Verifica se já preencheu os 74 bytes (dados + RSSI)
      if (indice_buffer >= TAMANHO_LORA) {
        
        // O último byte no buffer (índice 73) é o RSSI
        // uint8_t raw_rssi = buffer_rx[73];
        // Em módulos EByte, o cálculo para dBm geralmente é: int rssi_dbm = -(256 - raw_rssi);
        
        // Passo 3: Envia os 74 bytes para o PC via USB
        Serial.write(buffer_rx, TAMANHO_LORA);
